...
class SmartElevatorAlgo:
    ...
    def elevator_assignment(self):
        new_list = []
        ID = 0
        if self._building.number_of_elevator == 1:
            for k in range(len(self.list_of_call.Call)):
                self.list_of_call.Call[k].index = 0
                self.list_of_call.Call[k].id = 0


        else:
            for j in range(len(self.list_of_call.Call)):
                curr = self.list_of_call.Call[j]
                if curr.id == -1:
                    curr.id = ID
                    ID += 1
                    k = j + 1
                    i = j
                    for k in range(len(self.list_of_call.Call)):  # **
                        curr_temp = self.list_of_call.Call[i]
                        next_curr = self.list_of_call.Call[k]
                        if next_curr.id == -1:
                            if curr_temp.direction() == next_curr.direction():
                                if curr_temp.dest == next_curr.dest:
                                    if curr_temp.direction() == 1:  # up
                                        if self.check_path(curr_temp, next_curr, k):  # go to next for
        ...
        for k in range(len(self.list_of_call.Call)):
            e = -1
            if self.list_of_call.Call[k].index == -1:
                for i in range(len(self.building.elevators)):
                    min_time = sys.maxsize
                    t1 = self.building.elevators[i].time()
                    t2 = self.building.elevators[i].time_of_path(self.list_of_call.Call[k].src,
                                                                 self.list_of_call.Call[k].dest)
                    last_call = self.building.elevators[i].list_elev[len(self.building.elevators[i].list_elev) - 1]
                    t3 = self.building.elevators[i].time_of_path(last_call.dest, self.list_of_call.Call[k].src)
                    t123 = t1 + t2 + t3
                    if t123 < min_time:
                        min_time = t123
                        e = i
                ...
                while m > 0:
                    if self.list_of_call.Call[m].id == self.list_of_call.Call[m - 1].id:
                        if self.list_of_call.Call[m - 1].src <= call2.src:
                            self.check_time(self.list_of_call.Call[m - 1], call2)
                            return True
                    m -= 1
        else:
            if call1.src >= call2.src:
    ...
    def check_time(self, call1, call2) -> int:
        for m in range(len(self.building.elevators)):
